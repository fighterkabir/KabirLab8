...
function initializeServiceWorker() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    window.addEventListener('load', function () {
      ...
      // **Citation note: this code was taken from MDN article "Using Service Workers"
...
async function getRecipes() {
  ...
  const currRecipes = JSON.parse(localStorage.getItem('recipes'));
  if (currRecipes != null) {
    return currRecipes;
  }
  /**************************/
    ...
    for (let i = 0; i < RECIPE_URLS.length; i++) {
      ...
      const fetchRecipeURLs = async () => {
        try {
          ...
          let fetchedJSON = await fetchedURL.json();
          arrOfRecipesFetched.push(fetchedJSON);
